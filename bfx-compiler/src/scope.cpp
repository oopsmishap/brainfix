...
std::string Scope::enclosing() const
{
    ...
    return d_stack.back().second.empty()
             ? ""
             : curr.substr(0, curr.find_last_of("::") - 1);
}
...
{
    assert(
        name == function() && "trying to exit function-scope other than current function"
    );
...
void Scope::push(Type type)
{
    ...
    d_stack.back().second.push_back({.type = type, .id = ++counter});
}
