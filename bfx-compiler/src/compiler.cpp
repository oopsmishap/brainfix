...
int Compiler::lex()
{
    ...
    case STR:
    {
        d_val_.assign<Tag_::STRING>(d_lexer.matched());
        break;
    }
    case NUM:
    {
        d_val_.assign<Tag_::INT>(std::stoi(d_lexer.matched()));
        break;
    }
    case HEX:
    {
        d_val_.assign<Tag_::INT>(std::stoi(d_lexer.matched(), nullptr, 16));
        break;
    }
    case CHR:
    {
        d_val_.assign<Tag_::CHAR>(d_lexer.matched()[0]);
        break;
    }
    default:
        break;
...
void Compiler::pushStream(std::string const& file)
{
    ...
    if (std::find(d_included.begin(), d_included.end(), shortFile)
        != d_included.end())
    {
        compilerWarningIf(
            d_includeWarningEnabled,
...
int Compiler::compile()
{
    ...
    compilerErrorIf(
        d_functionMap.find(BFXFunction::mangle("main", 0)) == d_functionMap.end(),
...
void Compiler::writeProfile() const
{
    ...
    file << "Profile for " << d_sourceFile << ":\n"
         ...
         << "    random extension: "
         << (d_randomExtensionEnabled ? "enabled" : "disabled") << '\n'
         << '\n'
...

{
    ...
    auto const generateFilename = [&](std::string const& caseName) -> std::string
    {
    ...
    for (auto const& test : testBody)
    {
        ...
        compilerErrorIf(
            std::find(d_testVector.begin(), d_testVector.end(), base)
                != d_testVector.end(),
            "Multiple definitions of test-case <",
            caseName,
            "> in test-block <",
            testName,
            ">."
...
void Compiler::error()
{
    std::cerr << "ERROR: Syntax error on line " << d_lexer.lineNr()
              << " of file " << d_lexer.filename() << '\n';
}
...
void Compiler::addFunction(BFXFunction const& bfxFunc)
{
    compilerErrorIf(
        ...
        "Duplicate parameters used in the definition of function \"",
        bfxFunc.name(),
        "\"."
    );
...
)
{
    ...
    for (auto const& pr : fields)
    {
        ...
        compilerErrorIf(
            ...
            "Field \"",
            fieldName,
            "\" previously declared in definition of struct \"",
            name,
            "\"."
        );
        compilerErrorIf(
            !fieldType.defined(), "Variable \'", fieldName, "\' declared with undefined (struct) type."
        );

        int const sz = fieldType.size();
        compilerErrorIf(sz == 0, "Cannot declare field \"", fieldName, "\" of size 0.");
        compilerErrorIf(
            fieldType.isIntType() && sz > MAX_ARRAY_SIZE,
            "Maximum array size (",
            MAX_ARRAY_SIZE,
            ") exceeded in struct definition (got ",
            sz,
            ")."
        );
    }
...
void Compiler::addGlobals(
    std::vector<std::pair<std::string, TypeSystem::Type>> const& declarations
)
{
    for (auto const& var : declarations)
    {
        ...
        compilerErrorIf(
            type.size() <= 0, "Global declaration of \"", ident, "\" has invalid size specification."
        );
...
{
    compilerWarningIf(
        num > MAX_INT, "use of value ", num, " exceeds limit of ", MAX_INT, "."
    );
    auto result = d_constMap.insert({ident, num});
    compilerErrorIf(
        !result.second, "Redefinition of constant ", ident, " is not allowed."
    );
}
...
{
    compilerErrorIf(
        !isCompileTimeConstant(ident),
        ident,
        " is being used as a const but was not defined as such."
    );
...
int Compiler::allocate(std::string const& ident, TypeSystem::Type type)
{
    ...
    {
        compilerErrorIf(
            addr < 0, "Variable ", ident, ": variable previously declared."
        );
    }
    else if (addr < 0)
    {
...
int Compiler::staticAssert(Instruction const& check, std::string const& msg)
{
    ...
    compilerErrorIf(
        !d_memory.valueKnown(result), "Could not evaluate static_assert at compiletime."
    );
...
int Compiler::statement(Instruction const& instr)
{
    if (d_bcrEnabled)
    {
        ...
        Instruction const condition = [&, this]()
        {
            return logicalAnd(continueFlag, breakFlag);
        };
...
int Compiler::call(std::string const& name, std::vector<Instruction> const& args)
{
    ...
    compilerErrorIf(
        d_functionMap.find(mangled) == d_functionMap.end(),
    ...
    compilerErrorIf(
        d_scope.containsFunction(mangled),
        "Function \"",
        name,
        "\" is called recursively. Recursion is not allowed."
    );
    ...
    for (size_t idx = 0; idx != args.size(); ++idx)
    {
        ...
        compilerErrorIf(
            argAddr < 0,
            "Invalid argument argument to function \"",
            func.name(),
            "\": the expression passed as argument ",
            idx,
            " returns void."
        );
        ...
        if (paramType == BFXFunction::ParameterType::Value)
        {
            ...
            int const paramAddr = d_memory.allocate(
                paramIdent, func.mangled(), d_memory.type(argAddr)
            );
    ...
    if (!(func.isVoid() || returnVariableIsReferenceParameter))
    {
        ...
        compilerErrorIf(
            ret == -1, "Returnvalue \"", retVar, "\" of function \"", func.name(), "\" seems not to have been declared in the main scope of the function-body."
        );
...
{
    compilerErrorIf(
        type.isNullType(), "Alias-variable ", ident, " cannot be declared without being initialized."
    );
    compilerErrorIf(
        !type.defined(), "Variable \'", ident, "\' declared with unknown type."
    );

    int const sz = type.size();
    compilerErrorIf(sz == 0, "Cannot declare variable \"", ident, "\" of size 0.");
    compilerErrorIf(
        sz < 0, "Size must be specified in declaration without initialization of variable ", ident
    );

    compilerErrorIf(
        type.isIntType() && sz > MAX_ARRAY_SIZE,
        "Maximum array size (",
        MAX_ARRAY_SIZE,
        ") exceeded (got ",
        sz,
...
int Compiler::initializeExpression(
...
)
{
    ...
    {
        compilerErrorIf(
            d_memory.find(ident, d_scope.current(), false) != -1,
            "Variable ",
            ident,
            " previously declared."
        );
    ...
    if (type.isNullType())
    {
        // Variable was declared without a type, which means it's an alias
        // variable and no new memory needs to be allocated.
        compilerErrorIf(rhs < 0, "Use of void expression in assignment.");
        compilerErrorIf(
            d_memory.isTemp(rhs), "Cannot create alias to temporary value."
        );
        if (!d_loopUnrolling)
    ...
    // A small optimization can be performed when the rhs cell contains a
    // temporary: the temp can simply be renamed to avoid the copy. This only
    // works when not in the middle of loop-unrolling.

    TypeSystem::Type rhsType = d_memory.type(rhs);
    if (d_memory.isTemp(rhs) && (sz == -1 || type == rhsType) && !d_loopUnrolling)
    {
        // In this situation, we can simply rename the temporary variable that
        // resulted from evaluating rhs to the declared variable.
    ...
    {
        // rhs is not a temp, so we need to copy the result into a newly
        // allocated variable.
        return assign(allocate(ident, (sz != -1) ? type : rhsType), rhs);
    }

    // If none of the cases matched, this means there's a type-mismatch between
    // the lhs and rhs of the assignment.
    compilerError(
        "Type mismatch in assignment of \"",
        rhsType.name(),
        "\" to variable \"",
        ident,
        "\" of type \"",
...
int Compiler::assign(AddressOrInstruction const& lhs, AddressOrInstruction const& rhs)
{
    ...
    compilerErrorIf(
        (leftType.isStructType() || rightType.isStructType())
            && !(leftType == rightType),
    ...
    compilerErrorIf(
        leftSize != rightSize && leftSize != 1 && rightSize != 1,
        "Assignment to array of size ",
        leftSize,
        " with object of incompatible size ",
        rightSize,
        "."
    );
...
)
{
    ...
    compilerError(
        "Structure \"", type.name(), "\" does not contain field \"", expr[baseIdx + 1], "\"."
    );
    return -1;
}
...
int Compiler::anonymousStructObject(
...
)
{
    ...
    compilerErrorIf(
        ...
        "Too many field-initializers provided to struct \"",
        name,
        "\": ",
        "expects ",
        fields.size(),
...
int Compiler::fetchElement(
    AddressOrInstruction const& arr, AddressOrInstruction const& index
)
{
    int const indexValue = d_memory.value(index);
    int const sz         = d_memory.sizeOf(arr);
    compilerWarningIf(
        d_boundsCheckingEnabled && indexValue >= sz,
        "Array index (",
        indexValue,
        ") out of bounds: sizeof(",
        d_memory.identifier(arr),
        ") = ",
        sz,
        "."
    );
...
)
{
    ...
    else
    {
        ...
        // Attention: can't return the address of the modified cell, so we
        // return the address of the known RHS-cell.
        return rhs;
    }
}

int Compiler::applyUnaryFunctionToElement(
    AddressOrInstruction const& arr, AddressOrInstruction const& index, UnaryFunction func
)
{
...
int Compiler::applyBinaryFunctionToElement(
    AddressOrInstruction const& arr,
    AddressOrInstruction const& index,
    AddressOrInstruction const& rhs,
    BinaryFunction              func
...
int Compiler::randomCell()
{
    ...
    {
        compilerWarning("Random number generation is implemented using the "
                        "non-standard 'Random Brainf*ck' extension "
                        "(https://esolangs.org/wiki/Random_Brainfuck). Your "
                        "interpreter must support the '?'-symbol.\nThis "
                        "warning can be supressed with the --random flag.");
        warned = true;
    }
...
int Compiler::postIncrement(AddressOrInstruction const& target)
{
    ...
    auto      bf  = [&, this]()
    {
        d_codeBuffer << d_bfGen.assign(tmp, target) << d_bfGen.incr(target);
    };
    auto func = [](int& x)
    {
        return x++;
...
int Compiler::add(AddressOrInstruction const& lhs, AddressOrInstruction const& rhs)
{
    ...
    auto      bf  = [&, this]()
    {
        d_codeBuffer << d_bfGen.assign(ret, lhs) << d_bfGen.addTo(ret, rhs);
    };
...
int Compiler::subtract(AddressOrInstruction const& lhs, AddressOrInstruction const& rhs)
{
    ...
    auto      bf  = [&, this]()
    {
        d_codeBuffer << d_bfGen.assign(ret, lhs) << d_bfGen.subtractFrom(ret, rhs);
    };
...
void Compiler::divModPair(
    AddressOrInstruction const& num,
    AddressOrInstruction const& denom,
    int const                   divResult,
    int const                   modResult
)
{
...
int Compiler::greaterOrEqual(
    AddressOrInstruction const& lhs, AddressOrInstruction const& rhs
)
{
    compilerErrorIf(lhs < 0 || rhs < 0, "Use of void-expression in comparison.");
...
int Compiler::logicalAnd(AddressOrInstruction const& lhs, AddressOrInstruction const& rhs)
{
    ...
    compilerErrorIf(
        d_memory.value(lhs) && rhs < 0, "Use of void-expression in and-operation."
    );
    ...
    // RHS will be evaluated, even if it won't be at runtime due to short
    // circuiting This will result in a warning if it contains an out-of bounds
    // index. Suppress this warning by disabling this check.
...
int Compiler::logicalOr(AddressOrInstruction const& lhs, AddressOrInstruction const& rhs)
{
    ...
    compilerErrorIf(
        d_memory.value(lhs) && rhs < 0, "Use of void-expression in or-operation."
    );
...
void Compiler::allocateBCRFlags(bool const alloc)
{
    ...
    else
    {
        ...
        assert(
            !enclosingScope.empty()
            && "calling allocateBCRFlags(false) without being in a subscope"
        );
    ...
    auto const result =
        d_bcrMap.insert({d_scope.current(), {breakFlag, continueFlag}});
...
)
{
    ...
    while (d_memory.value(conditionAddr))
    {
        ...
        resetContinueFlag();
        increment();
        conditionAddr = d_bcrEnabled
                          ? logicalAnd(condition, getCurrentBreakFlag())
                          : condition();
        ++d_loopUnrolling;

        if (!d_memory.valueKnown(conditionAddr)
            || ++count > MAX_LOOP_UNROLL_ITERATIONS)
        {
            restore(std::move(state));
            return forStatementRuntime(init, condition, increment, body);
        }
...
int Compiler::forStatementRuntime(
    Instruction const& init,
    Instruction const& condition,
    Instruction const& increment,
    Instruction const& body
)
{
    ...
    resetContinueFlag();
    increment();
    conditionAddr = d_bcrEnabled ? logicalAnd(condition, getCurrentBreakFlag())
                                 : condition();
...
int Compiler::forRangeStatement(
    BFXFunction::Parameter const& param, Instruction const& array, Instruction const& body
)
{
...
int Compiler::forRangeStatementRuntime(
    BFXFunction::Parameter const& param, Instruction const& array, Instruction const& body
)

{
    auto const& [ident, paramType] = param;
    compilerWarningIf(
        paramType == BFXFunction::ParameterType::Reference, "Declaring ranged-for variable as reference has no effect in runtime loop."
    );
    ...
    int const elementAddr = declareVariable(ident, TypeSystem::Type(1));
    compilerErrorIf(
        elementAddr < 0 || arrayAddr < 0, "Use of void-expression in for-initialization."
    );
    ...
    int conditionAddr     = d_bcrEnabled
                              ? logicalAnd(finalElementCheck, getCurrentBreakFlag())
                              : finalElementCheck;
...
int Compiler::whileStatementRuntime(Instruction const& condition, Instruction const& body)
{
    ...
    resetContinueFlag();
    int conditionAddr =
        d_bcrEnabled ? logicalAnd(condition, getCurrentBreakFlag()) : condition();
...
)
{
    ...
    ifElseLadder = [&, this](size_t const idx) -> Instruction
    {
        if (idx == cases.size() - 1)
        {
...
{
    compilerErrorIf(
        !d_bcrEnabled, "break-statement not supported when compiling with --no-bcr"
    );
...
{
    compilerErrorIf(
        !d_bcrEnabled, "continue-statement not supported when compiling with --no-bcr"
    );
...
{
    compilerErrorIf(
        !d_bcrEnabled, "return-statement not supported when compiling with --no-bcr"
    );
...
{
    auto cancel = [](std::string const& input, char const up, char const down
                  ) -> std::string
    {
        std::string result;
        int         count = 0;
        ...
        {
            if (count > 0)
                result += std::string(count, up);
            if (count < 0)
                result += std::string(-count, down);
            count = 0;
        };
