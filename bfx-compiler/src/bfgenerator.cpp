...
std::string BFGenerator::assign(int const lhs, int const rhs)
{
    ...
    ops << setToValue(lhs, 0)
        ...
        << movePtr(rhs) << "[" << incr(lhs) << incr(tmp) << decr(rhs)
        << "]"

        // Restore RHS by moving TMP back into it
        << movePtr(tmp) << "[" << incr(rhs) << decr(tmp)
        << "]"
...
std::string BFGenerator::addTo(int const target, int const rhs)
{
    ...
    std::ostringstream ops;
    int const          tmp = f_getTemp();
    ops << assign(tmp, rhs) << "[" << incr(target) << decr(tmp) << "]"
        << movePtr(target);
...
std::string BFGenerator::subtractFrom(int const target, int const rhs)
{
    ...
    int const          tmp = f_getTemp();
    std::ostringstream ops;
    ops << assign(tmp, rhs) << "[" << decr(target) << decr(tmp) << "]"
        << movePtr(target);
...
std::string BFGenerator::safeDecr(int const target, int const underflowFlag)
{
    ...
    std::ostringstream ops;
    ops << logicalNot(target, underflowFlag) << movePtr(target) << "-";
...
std::string BFGenerator::multiply(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << assign(result, lhs) << multiplyBy(result, rhs);
...
std::string BFGenerator::multiplyBy(int const target, int const factor)
{
    ...
    std::ostringstream ops;
    ops << assign(targetCopy, target) << setToValue(target, 0)
        << assign(count, factor) << "[" << addTo(target, targetCopy)
        << decr(count) << "]" << movePtr(target);
...
std::string BFGenerator::power(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << assign(result, lhs) << powerBy(result, rhs);
...
std::string BFGenerator::powerBy(int const base, int const pow)
{
    ...
    std::ostringstream ops;
    ops << assign(baseCopy, base) << setToValue(base, 1) << assign(powCopy, pow)
        << "[" << multiplyBy(base, baseCopy) << decr(powCopy) << "]"
        << movePtr(base);
...
std::string BFGenerator::logicalNot(int const addr, int const result)
{
    ...
    ops << setToValue(result, 1) << assign(tmp, addr) << "["
        << setToValue(result, 0) << setToValue(tmp, 0) << "]" << movePtr(result);
...
std::string BFGenerator::logicalNot(int const addr)
{
    ...
    std::ostringstream ops;
    ops << setToValue(flag, 1) << movePtr(addr) << "[" << setToValue(flag, 0)
        << setToValue(addr, 0) << "]" << movePtr(flag) << "["
        << setToValue(addr, 1) << setToValue(flag, 0) << "]" << movePtr(addr);
...
std::string BFGenerator::logicalAnd(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << setToValue(result, 0) << assign(y, rhs) << assign(x, lhs) << "["
        << movePtr(y) << "[" << setToValue(result, 1) << setToValue(y, 0) << "]"
        << setToValue(x, 0) << "]" << movePtr(result);
...
std::string BFGenerator::logicalAnd(int const lhs, int const rhs)
{
    ...
    std::ostringstream ops;
    ops << logicalAnd(lhs, rhs, result) << assign(lhs, result);
...
std::string BFGenerator::logicalOr(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << setToValue(result, 0) << assign(x, lhs) << "[" << setToValue(result, 1)
        << setToValue(x, 0) << "]" << assign(y, rhs) << "["
        << setToValue(result, 1) << setToValue(y, 0) << "]" << movePtr(result);
...
std::string BFGenerator::logicalOr(int const lhs, int const rhs)
{
    ...
    std::ostringstream ops;
    ops << logicalOr(lhs, rhs, result) << assign(lhs, result);
...
std::string BFGenerator::equal(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << setToValue(result, 1) << setToValue(underflow1, 0) << assign(y, rhs)
        << assign(x, lhs) << "[" << safeDecr(y, underflow2)
        << logicalOr(underflow1, underflow2) << movePtr(underflow2) << "["
        << setToValuePlus(y, 0) << setToValue(underflow2, 0) << "]" << decr(x)
        << "]" << assign(underflow3, underflow1)
        << "[" // if underflow -> y was smaller than x so not equal
        << setToValue(result, 0) << setToValuePlus(y, 1)
        << setToValue(underflow3, 0) << "]" << logicalNot(underflow1)
        << logicalAnd(y, underflow1, yBigger)
        << "[" // if y > 0 and did not underflow -> y was bigger than x so not
               // equal
        << setToValue(result, 0) << setToValue(yBigger, 0) << "]"
        << movePtr(result);
...
std::string BFGenerator::notEqual(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << equal(lhs, rhs, isEqual) << logicalNot(isEqual, result);
...
std::string BFGenerator::greater(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << setToValue(result, 0) << setToValue(underflow, 0) << assign(y, rhs)
        << assign(x, lhs) << "[" << safeDecr(y, underflow)
        << logicalOr(result, underflow) << movePtr(underflow) << "["
        << setToValuePlus(y, 0) << setToValue(underflow, 0) << "]" << decr(x)
        << "]" << movePtr(result);
...
std::string BFGenerator::greaterOrEqual(int const lhs, int const rhs, int const result)
{
    ...
    std::ostringstream ops;
    ops << equal(lhs, rhs, isEqual) << greater(lhs, rhs, isGreater)
...
std::string BFGenerator::fetchElement(
    int const arrStart, int const arrSize, int const index, int const ret
)
{
    ...
    std::ostringstream ops;
    ops << assign(buf + 0, index) << assign(buf + 1, buf)
        << setToValue(buf + 2, 0, bufSize - 2) << movePtr(buf)
        << dynamicMoveRight << buf2arr << "[-" << arr2buf << ">>+<<" << buf2arr
        << "]" << arr2buf << ">>"
        << "["
        << "-<<+" << buf2arr << "+" << arr2buf << ">>"
        << "]"
        << "<" << dynamicMoveLeft << assign(ret, buf);

    return ops.str();
}

std::string BFGenerator::assignElement(
    int const arrStart, int const arrSize, int const index, int const val
)
{
    static std::string const dynamicMoveRight = "[>>[->+<]<[->+<]<[->+<]>-]";
    static std::string const dynamicMoveLeft  = "[[-<+>]<-]<";
    ...
    std::ostringstream ops;
    ops << assign(buf, index) << assign(buf + 1, buf) << assign(buf + 2, val)
        << setToValue(buf + 3, 0, bufSize - 3) << movePtr(buf)
        << dynamicMoveRight << buf2arr << "[-]" << arr2buf << ">>"
        << "["
        << "-<<" << buf2arr << "+" << arr2buf << ">>"
        << "]"
        << "<" << dynamicMoveLeft;
...
std::string BFGenerator::divmod(
    int const num, int const denom, int const divResult, int const modResult
)
{
    ...
    // 2. In case the denominator is 0 (divide by zero), set the result to 255
    // (~inf)
    //    Set the loopflag to 0 in order to skip the calculating loop.
    // 3. In case the numerator is 0, the result of division is also zero. Set
    // the remainder
    //    to the same value as the denominator.
    // 4. Enter the loop:
    //      *  On each iteration, decrement both the denominator and enumerator,
    //       until the denominator becomes 0.When this happens, increment
    //       result_div and reset result_mod to 0. Also, reset the denominator
    //       to its original value.
    //
    //    *  The loop is broken when the enumerator has become zero.
    //       By that time, we have counted how many times te denominator
    //       fits inside the enumerator (result_div), and how many is left
    //       (result_mod).

    std::ostringstream ops;
    ops << setToValue(divResult, 0) // 1
        << setToValue(modResult, 0) << assign(tmp_num, num)
        << assign(tmp_denom, denom) << setToValue(tmp_loopflag, 1)
        << logicalNot(denom, tmp_zeroflag) << "[" // 2
        << setToValue(tmp_loopflag, 0) << setToValue(divResult, d_maxCellValue)
        << setToValue(modResult, d_maxCellValue) << setToValue(tmp_zeroflag, 0)
        << "]" << logicalNot(num, tmp_zeroflag) << "[" // 3
        << setToValue(tmp_loopflag, 0) << setToValue(divResult, 0)
        << setToValue(modResult, 0) << setToValue(tmp_zeroflag, 0) << "]"
        << movePtr(tmp_loopflag) << "[" // 4
        << decr(tmp_num) << decr(tmp_denom) << incr(modResult)
        << logicalNot(tmp_denom, tmp_zeroflag) << "[" << incr(divResult)
        << assign(tmp_denom, denom) << setToValue(modResult, 0)
        << setToValue(tmp_zeroflag, 0) << "]" << logicalNot(tmp_num, tmp_zeroflag)
        << "[" << setToValue(tmp_loopflag, 0) << setToValue(tmp_zeroflag, 0)
