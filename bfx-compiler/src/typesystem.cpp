...
class StructDefinition
{
  ...
  public:
    StructDefinition(std::string const& name) : d_size(1), d_name(name)
    {
    }

    StructDefinition() : d_valid{false}
    {
    }
...
bool TypeSystem::add(
    std::string const& name, std::vector<std::pair<std::string, Type>> const& fields
)
{
