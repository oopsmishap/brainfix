...
class BFGenerator
{
  ...
  public:
    BFGenerator(size_t maxCellValue = 0xff) : d_maxCellValue(maxCellValue)
    {
    }
    ...
    template <typename GetTemp> void setTempRequestFn(GetTemp&& getTemp)
    {
    ...
    std::string powerBy(int const lhs, int const rhs);
    std::string
    divmod(int const num, int const denom, int const divResult, int const modResult);
