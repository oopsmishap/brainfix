...
private:
    ...
    size_t (LexerBase::*d_get)() = &LexerBase::getInput;
...
protected:
    ...
    int     const (*d_dfaBase_)[70];

    static int     const s_dfa_[][70];
    static int     const (*s_dfaBase_[])[70];
...
public:
    LexerBase(LexerBase const &other)             = delete;
...
protected:
    LexerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    LexerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~LexerBase();
...
inline LexerBase::~LexerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr LexerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline LexerBase::StartCondition_ constexpr LexerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr LexerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline LexerBase::StartCondition_ LexerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void LexerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool LexerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr LexerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &LexerBase::out()
{
    return *d_out;
}

inline void LexerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void LexerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<LexerBase::StreamStruct> const &LexerBase::streamStack() const
{
...
inline void LexerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void LexerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &LexerBase::matched() const
{
    return d_matched;
}

inline std::string const &LexerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &LexerBase::filename() const
{
    return d_filename;
}

inline void LexerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t LexerBase::length() const
{
    return d_matched.size();
}

inline void LexerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t LexerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void LexerBase::more()
{
    d_more = true;
}

inline size_t LexerBase::state_() const
{
    return d_state;
}

inline size_t LexerBase::get_()
{
    return (this->*d_get)();
}

inline size_t LexerBase::getInput()
{
    return d_input->get();
}

inline bool LexerBase::return_()
{
    return d_return;
}

inline void LexerBase::noReturn_()
{
    d_return = false;
}
